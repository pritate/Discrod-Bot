...
# ---------------- CONFIG ----------------
ALLOWED_CHANNELS = [1425720821477015553, 1427263126989963264]  # 🔸 Add both channel IDs here
...
# ---------------- TRACKING ----------------
user_sent_times = {}
global_next_spawn = {}         # key: (channel_id, spawn_name)
spawn_warned = {}
upcoming_msg_id = {}
card_auto_extended = {}
...
def get_member_timezone(member: discord.Member) -> ZoneInfo:
    for role in member.roles:
        if role.name.upper() in ROLE_TIMEZONES:
            return ZoneInfo(ROLE_TIMEZONES[role.name.upper()])
    return PHT
...
def build_upcoming_message(channel_id):
    lines = []

    # Rooms
    room_lines = []
    for (cid, key), spawn_time in global_next_spawn.items():
        if cid != channel_id or key not in ROOM_NAMES:
            continue
    ...
    boss_lines = []
    for (cid, key), spawn_time in global_next_spawn.items():
        if cid != channel_id or key not in BOSS_NAMES:
            continue
        unix_ts = unix_timestamp(spawn_time)
    ...
    card_lines = []
    for (cid, key), spawn_time in global_next_spawn.items():
        if cid != channel_id or not (key.startswith("PCARD") or key.startswith("BCARD")):
            continue
...
async def update_upcoming_message(channel: discord.TextChannel):
    content = build_upcoming_message(channel.id)
...
async def five_minute_warning():
    ...
    for (cid, key), spawn_time in list(global_next_spawn.items()):
        if (cid, key) not in spawn_warned and 0 <= (spawn_time - now).total_seconds() <= 300:
            channel = bot.get_channel(cid)
            warn_msg = await channel.send(f"@everyone ⚠️ **{key}** will spawn in 5 minutes!")
            spawn_warned[(cid, key)] = True
            bot.loop.create_task(delete_later(warn_msg, 300))
...
async def cleanup_expired_messages():
    ...
    for (cid, key), spawn_time in list(global_next_spawn.items()):
        ...
        if now >= expire_time:
            expired.append((cid, key))

    for cid, key in expired:
        channel = bot.get_channel(cid)
        spawn_time = global_next_spawn[(cid, key)]
        expired_msg = await channel.send(
            f"❌ {key} expired (Spawned at {spawn_time.strftime('%I:%M %p')})"
        )
        bot.loop.create_task(delete_later(expired_msg, 300))
        del global_next_spawn[(cid, key)]
        spawn_warned.pop((cid, key), None)
        card_auto_extended.pop((cid, key), None)
...
async def extend_card_time():
    ...
    for (cid, key), spawn_time in list(global_next_spawn.items()):
        if key.startswith("PCARD") or key.startswith("BCARD"):
            if (cid, key) not in card_auto_extended:
                if now >= spawn_time and (now - spawn_time).total_seconds() < 60:
                    global_next_spawn[(cid, key)] = spawn_time + timedelta(minutes=30)
                    card_auto_extended[(cid, key)] = True

async def delete_later(msg, delay):
    await discord.utils.sleep_until(datetime.now(timezone.utc) + timedelta(seconds=delay))
    try:
        await msg.delete()
    except discord.NotFound:
        pass
...
async def on_message(message):
    ...
    for match in time_pattern.finditer(message.content):
        ...
        if key in BOSS_NAMES:
            ...
            continue
        if (channel_id, key) in global_next_spawn and global_next_spawn[(channel_id, key)] == next_spawn:
        ...
        if key in user_sent_times[user_id] and user_sent_times[user_id][key] == next_spawn:
            await message.delete()
            warn = await message.channel.send(
                f"⚠️ You already sent the same time for {key}.",
                delete_after=5
            )
            continue
        user_sent_times[user_id][key] = next_spawn
        global_next_spawn[(channel_id, key)] = next_spawn
        updated = True
    ...
    for match in card_pattern.finditer(message.content):
        ...
        full_loc = f"{loc1} {loc2}".strip().upper()
        if full_loc in CARD_LOCATIONS:
            ...
            if (channel_id, key) in global_next_spawn and global_next_spawn[(channel_id, key)] == next_spawn:
                ...
                await message.channel.send(
                    f"⚠️ The next spawn for {key} is already posted!", delete_after=6
                )
                continue
            if key in user_sent_times[user_id] and user_sent_times[user_id][key] == next_spawn:
                await message.delete()
                await message.channel.send(
                    f"⚠️ You already sent the same time for {key}.", delete_after=5
                )
                continue
            user_sent_times[user_id][key] = next_spawn
            global_next_spawn[(channel_id, key)] = next_spawn
            card_auto_extended.pop((channel_id, key), None)
            updated = True
